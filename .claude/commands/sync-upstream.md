...
echo "      ~/ai/agent-config/claude/RTK.md           (source of ~/.claude/RTK.md and ~/.cursor/RTK.md — hand-edited)"
echo "      ~/ai/agent-config/codex/AGENTS.md         (source of ~/.codex/AGENTS.md — hand-edited)"
echo "      ~/ai/agent-config/codex/RTK.md            (source of ~/.codex/RTK.md; antigravity/RTK.md symlinks here)"
echo "      hooks/rtk-awareness{,-high,-full}.md      (this repo) (shared awareness templates, one per awareness.level)"
...
3. **Doc update analysis.** After a successful rebase, scan the upstream
   ...
   - **Agent rules live in `~/ai/agent-config/`, not under the dotfile
     directories.** `~/.claude/RTK.md`, `~/.cursor/RTK.md`, `~/.codex/RTK.md`,
     `~/.codex/AGENTS.md`, and `~/.gemini/config/RTK.md` are all symlinks into
     that repo, so edit the source and commit there. Check the symlink target
     before editing; `rtk init` would overwrite a real file at those paths.
     - `claude/RTK.md` — feeds Claude Code and Cursor. Behavior notes the
       model needs at generation time (pipeline rewrite scope, wrapper
       peeling, bypass discipline, new subcommand semantics).
     - `codex/RTK.md` — feeds Codex CLI; `antigravity/RTK.md` is a symlink
       to it, so one edit covers Gemini too.
     - `codex/AGENTS.md` and `antigravity/AGENTS.md` — each carries its own
       inline RTK paragraph. They are separate files with near-identical
       text, so grep both for the sentence you are correcting.
     - `~/.config/opencode/RTK.md` is a real file for an unused harness.
       Leave it alone.
   - **`hooks/rtk-awareness.md`, `-high`, `-full`** (this repo) — the shared
     awareness templates, one per `awareness.level`, replacing the per-agent
     `hooks/<agent>/rtk-awareness.md` files upstream deleted in 0.49.0. Agents
     without a command hook (Codex CLI, Antigravity, Cline, Windsurf, Kilo
     Code, Kimi) always get `-full` regardless of configured level, because
     only that variant teaches self-prefixing.
     **`test_awareness_files_stay_within_line_budget` caps these at 10 / 21 /
     25 lines and `-full` already sits at 25.** Guidance that does not fit
     belongs in the local agent-config files, not in a raised cap.
   - **`~/ai/wiki/tools/rtk.md`** — cross-repo wiki entry. Always append
     ...
     depends on it). Update the upstream version and the material-paths
     table in `## Current supported build`, and drop any row upstream has
     absorbed. After editing, run `.tools/validate-wiki.py`, lint the page,
     and run the natural-language recall probe STYLE.md requires — material
     buried inside an unrelated H2 often fails it and needs promoting.

   Triggers (non-exhaustive):
   - Hook / init behavior change (`transparent_prefixes`, pipeline producer
     rewriting, wrapper peeling, `--dry-run`, `permissionDecision` semantics,
     new agent target) → update every agent-config file carrying the affected
     claim, in parallel, plus the wiki. Verify the new behavior with
     `rtk rewrite '<command>'` against the freshly installed binary before
     writing it down; the source is not the installed build.
