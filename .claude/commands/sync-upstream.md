...
echo "      ~/.claude/RTK.md                  (global rtk usage rules)"
...
3. **Doc update analysis.** After a successful rebase, scan the upstream
   commit set for changes that may make existing docs stale:
   - New subcommands → may need entries in fork README "Filters supported"
     section and `src/cmds/<eco>/README.md`.
   - Hook / init behavior changes (e.g. `transparent_prefixes`,
     `--dry-run`, `permissionDecision` semantics) → may need a note in
     `~/.claude/RTK.md` or `~/ai/wiki/tools/rtk.md`.
   - Security pins / workflow restructuring → likely repo-internal only.
   - Breaking changes → CLAUDE.md and INSTALL.md.
