...
impl Counts {
    ...
    fn total(&self) -> usize {
        self.pass + self.fail + self.skip + self.bork + self.warn + self.leak + self.xfail
    }
...
fn parse(stripped: &str) -> Parsed {
    lazy_static::lazy_static! {
        ...
        static ref STATUS_RE: Regex = Regex::new(
            r"(?:^|\])(PASS|FAIL|SKIP|BORK|WARN|LEAK|XFAIL)\s+(.*?)\s*\[([^\[\]]+\.phpt)\]"
        ).unwrap();
        static ref STAT_RE: Regex = Regex::new(
            r"^\s*(Tests passed|Tests failed|Tests skipped|Tests warned|Expected fail|Tests leaked|Tests borked)\s*:\s*(\d+)"
        ).unwrap();
    ...
    let mut pending_diff: Option<Vec<String>> = None;
    ...
    for raw_line in stripped.lines() {
        ...
        if line == "========DONE========" {
            in_diff = false;
            pending_diff = Some(std::mem::take(&mut diff_buf));
            continue;
        }
        if in_diff {
        ...
        if let Some(cap) = STATUS_RE.captures(line) {
            ...
            if matches!(status, Status::Fail | Status::Bork | Status::Leak) {
                p.failures.push(Failure {
                    ...
                    diff: pending_diff.take().unwrap_or_default(),
                });
            } else {
                pending_diff = None;
            }
...
fn build_summary(p: &Parsed) -> String {
    ...
    if counts.xfail > 0 {
        out.push_str(&format!(", {} xfail", counts.xfail));
    }
    if counts.warn > 0 {
        out.push_str(&format!(", {} warn", counts.warn));
    }
    if counts.bork > 0 {
        out.push_str(&format!(", {} bork", counts.bork));
    }
    if counts.leak > 0 {
        out.push_str(&format!(", {} leak", counts.leak));
    }
    ...
    if counts.broken_total() == 0 || p.failures.is_empty() {
    ...
    for (i, f) in p.failures.iter().take(MAX_FAILURES_SHOWN).enumerate() {
        ...
        out.push('\n');
        for line in f.diff.iter().take(MAX_DIFF_LINES_PER_FAILURE) {
            out.push_str("    ");
            out.push_str(line);
            out.push('\n');
        }
        if f.diff.len() > MAX_DIFF_LINES_PER_FAILURE {
            out.push_str(&format!(
                "    ... +{} more diff lines\n",
                f.diff.len() - MAX_DIFF_LINES_PER_FAILURE
            ));
        }
    }
